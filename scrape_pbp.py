...
from scrape_urls import get_season_url
from NBA_Data_Jump_Ball_Player_Correction import JumpballPlayerCorrection
...
def play_caller(string):
    ...
    try:
        ...
        is_jump = ["", "", ""]
    return (
        is_shot + is_foul + is_rbd + is_vio + is_to + is_ft + is_ent + is_turn + is_jump
    )
...
def pbp2(link):
    ...
    plays = []
    dog = " ".join(
        soup.find("div", class_="scorebox")
        .find("div", class_="scorebox_meta")
        .find("div")
        .get_text()
        .split(", ")[1:]
    ).replace(",", "")
    ...
    awayTeamabbr, homeTeamabbr = [
        ...
        for x in [
            a["href"]
            for a in soup.find("div", class_="scorebox").find_all("a")
            if "teams" in a["href"]
        ]
    ]
    ...
    for i in range(2, len(soup.find("table", id="pbp").find_all("tr"))):
        try:
            ...
            if "Start of" in awayPlay.text:
                ...
                homePlay = (
                    soup.find("table", id="pbp").find_all("tr")[i].find_all("td")[5]
                )
                plays.append(
                    [
                        link,
            ...
            try:
                if len(plays) == 0:
                    ...
                    jb = (
                        soup.find("table", id="pbp").find_all("tr")[i].find_all("td")[1]
                    )
                    plays.append(
                        [
                            link,
...
def pbp_to_csv(year):
    y = open(f"NBA_PBP_{year - 1}-{year}.csv", "a")
    if os.stat(f"NBA_PBP_{year - 1}-{year}.csv").st_size == 0:
        y.write(
    ...
    for link in game_links:
        if t % 15 == 0 and t != 0:
            ...
            try:
                ...
                b = open(f"gamelinks_{year}.txt", "w")
                for link in list(set(game_links) - set(used_links)):
                    b.write(link + "\n")
                b.close()
                c = open(f"donelinks_{year}.txt", "a")
...
if __name__ == "__main__":
    ...
    t = open(f"gamelinks_{year}.txt", "r")
    try:
        donet = open(f"donelinks_{year}.txt", "r")
    except:
        donet = open(f"donelinks_{year}.txt", "w")
        donet.close()
        donet = open(f"donelinks_{year}.txt", "r")
    ...
    JumpballPlayerCorrection(f"NBA_PBP_{year - 1}-{year}.csv")
